    ...
    <>
      <nav className="fixed top-0 left-0 w-full z-[60] pt-6 px-6 pointer-events-none">
        <div className="max-w-5xl mx-auto flex justify-center">
          ...
          >
            {/* Brand Logo */}
            <Link href="/" className="flex items-center gap-3 relative z-[60]" onClick={() => setMenuOpen(false)}>
              <span className={`w-2.5 h-2.5 rounded-full ${menuOpen ? "bg-accent" : "bg-accent"} animate-pulse shadow-[0_0_10px_rgba(0,102,255,0.5)]`} />
              <span className={`text-xl font-black tracking-widest transition-colors duration-500 ${
            ...
            <div className="flex items-center gap-6 relative z-[60]">
              <a 
                ...
                className={`flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest transition-colors duration-500 hover:text-accent ${
                  menuOpen ? "text-white/70" : scrolled ? "text-text-muted" : "text-white/80"
                }`}
              >
                <MapPin size={14} />
              ...
              <div className={`w-[1px] h-4 ${menuOpen ? "bg-white/20" : scrolled ? "bg-border" : "bg-white/20"}`} />

              <button 
                onClick={() => setMenuOpen(!menuOpen)}
                className={`text-[10px] font-bold uppercase tracking-widest transition-colors duration-500 ${
                  menuOpen ? "text-white hover:text-accent" : scrolled ? "text-primary hover:text-accent" : "text-white hover:text-accent"
                }`}
              >
                {menuOpen ? "Close" : "Menu"}
              </button>
      ...
      {/* Full Screen Menu Overlay (Kept exactly the same as previous premium version) */}
      <AnimatePresence>
        {menuOpen && (
           <motion.div 
